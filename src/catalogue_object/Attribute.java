...
public class Attribute extends SortableCatalogueObject implements Mappable {
	...
	public Attribute( Catalogue catalogue, int id, String code, String name, String label, String scopenotes,
			String reportable, boolean visible, boolean searchable, int order, String type,
			int maxLength, int precision, int scale, String catalogueCode, String singleOrRepeatable,
			String inheritance, boolean uniqueness, boolean termCodeAlias, String status,
			String version, Timestamp lastUpdate, Timestamp validFrom, Timestamp validTo, boolean deprecated) {
		
		super( catalogue, id, code, name, label, scopenotes, order, status, version, 
				lastUpdate, validFrom, validTo, deprecated );
	...
	/**
	 * Check if the attribute is the implicit facet attribute, the one which represents the facetheader.facetfode$...
	 * @return
	 */
	...
	/**
	 * Check if the attribute is the all facet attribute, the one which represents the facetheader.facetfode$...
	 * with all the inherited facets!
	 * @return
	 */
	...
	/**
	 * check if the attribute is repeatable (i.e. it contains several values separated by $)
	 * @return
	 */
	...
	/**
	 * Check if the attribute type is catalogue. If this is the case
	 * then we have an implicit facet attribtue
	 * @return
	 */
	...
	public void setVariableByKey(String key, String value) throws WrongKeyException {
		...
		try {
			...
			case "type":
				setType( value ); break;
			case "reportable":
				setReportable( value ); break;
			case "visible":
				setVisible( Boolean.parseBoolean( value ) ); break;
			case "searchable":
				setSearchable( Boolean.parseBoolean( value ) ); break;
			case "maxlength":
				setMaxLength( Integer.parseInt( value ) ); break;
			case "precision":
				setPrecision( Integer.parseInt( value ) ); break;
			case "scale":
				setScale( Integer.parseInt( value ) ); break;
			case "catcode":
				setCatalogueCode( value ); break;
			case "single_repeatable":
				setSingleOrRepeatable( value ); break;
			case "inheritance":
				setInheritance( value ); break;
			case "uniqueness":
				setUniqueness( Boolean.parseBoolean( value ) ); break;
			case "termcodealias":
				setTermCodeAlias( Boolean.parseBoolean( value ) ); break;
			default:
				found = false; break;
			}
		}
	...
	public String getVariableByKey(String key) throws WrongKeyException {
		...
		try {
			...
			case "type":
				value = getType(); break;
			case "reportable":
				value = getReportable(); break;
			case "visible":
				value = String.valueOf( isVisible() ); break;
			case "searchable":
			...
			case "single_repeatable":
				value = getSingleOrRepeatable(); break;
			case "inheritance":
				value = getInheritance(); break;
			case "uniqueness":
				value = String.valueOf( isUniqueness() ); break;
			case "termcodealias":
				value = String.valueOf( isTermCodeAlias() ); break;
			default:
				found = false; break;
			}
		}
