...
public abstract class SheetWriter {
	...
	/**
	 * Set a progress bar which needs to be updated. Max fill is used to limit the maximum relative amount
	 * of progress that this sheet can achieve. The label is the displayed title.
	...
	/**
	 * Set the progress bar which needs to be updated. Max fill is used to limit the maximum relative amount
	 * of progress that this sheet can achieve.
	 * @param progressBar, the progress bar which is displayed in the main UI
	 * @param maxFill, the maximum relative amount that this sheet can be add to the progress bar
	 */
	...
	/**
	 * Get the list of elements which needs to be inserted into the
	 * sheet. Note that it is sufficient that the objects implements
	 * the method getValueByKey in order to be inserted into the
	 * sheet. In fact, we use the headers key as key to retrieve
	 * the fields of the object. For example, if we have as header
	 * headers.put( "CAT_CODE", new SheetHeader(0, "code" ) );
	 * we will use CAT_CODE to retrieve the code of the catalogue, in order
	 * to put it as value in the current row under the header "code" which is
	 * specified in the SheetHeader together with the column index.
	 * @return
	 */
