...
/**
 * This class is used to manage all the software related to the warnings in the describe function
 * In particular, it is necessary to initialize it with a table viewer of the warnings and a semaphore (canvas) 
 * in order to use this class. Example:
 * 	TableViewer warningTable = ...
 * 	Canvas semaphore = ...
 * 	WarningUtils warnUtils = new WarningUtils( warningTable, semaphore );
 * 
 * Calling from WarningUtils the method refreshWarningsTable will check if the full code of a term 
 * raise warnings or not ( and possibly it shows the related warnings in the warningTable ).
 * @author Valentino
 *
 */
...
public class WarningUtil {
	...
	private static enum WarningEvent {
		...
		WrongProcessOrder,          // when a process is added to a derivative and the process ordcode is < than the implicit ordcode 
		ExceptionTermSelected,      // when a term which is contained in the BR_Exceptions is selected for the describe function (they are ambiguous)
		MutuallyExPropertyViolated, // when more than one process with the same ord code is chosen
		NoRepNoExpBaseTerm,         // when a base term which does not belong to reporting or exposure hierarchy is selected
		NoExpHierarchyTerm,			// when a hierarchy is chosen as base term, and the hierarchy is not an exposure hierarchy
		...
		MixedDerivative,            // when a source is added to a derivative which has two or more source commodities
	...
	/**
	 * Check if the selected term belongs to a warn group
	 * ( i.e. a group which could raise a warning defined in BR_Data.csv or BR_Exceptions.csv )
	 * @param baseTerm
	...
	private Term getWarnGroup ( Term baseTerm, boolean stdOut ) {
		...
		while ( parent != null ) {
			...
			// get the parent of the current term and continue the loop, we use the reporting hierarchy for warnings
	...
	/**
	 * Check if the selected term identified by the groupCode is one
	 * of the warn groups ( i.e. a group which could raise a warning defined in BR_Data.csv or BR_Exceptions.csv )
	 * @param groupCode
	 * @return
	 */
	...
	private ArrayList<ForbiddenProcess> getImplicitForbiddenProcesses ( Term term,
		...
		while ( st.hasMoreTokens() ) {
			...
			// if it is a process and it is one of the dangerous ones, insert it in the array
	...
	/**
	 * Get the facet index and the facet code starting from a composite string which is like : F01.ADE0A
	 * @param facetFullCode
	 * @return
	 */
	...
	/**
	 * This method retrieves the forbidden processes related to a single warn group, which is given as input
	...
	/**
	 * Retrieve the current forbidden processes ord code starting from the current forbidden processes
	 * @param baseTerm
	 * @param currentFP
	 * @return
	 */
	@SuppressWarnings("unused")
	private ArrayList<Double> getCurrentForbiddenProcessesOrdCode ( Term baseTerm, ArrayList<ForbiddenProcess> currentFP ) { 
	...
	/**
	 * Retrieve the minimum ord code contained in the array list of forbidden processes
	...
	private boolean isGenericProcessFacet ( Term facet ) {
		return( facet.getCode().equals("A0C0R") || facet.getCode().equals("A0CHR") ||
	...
	/**
	 * SECOND WARNING CHECK
	 * Raise a warning if the user select a raw commodity and uses the describe function
	 * to create a derivative which is already present in the main list.
	 * In particular, the following code checks if the term belongs to one of the warn groups. If this
	 * is the case, it checks if any of the processes, which are added to the base term, generate a
	 * derivative term which is already present in the main list. ONLY FOR RAW COMMODITIES
	...
	/**
	 * THIRD WARNING CHECK
	 * Raise a warning if the user add to a derivative product an explicit process 
	 * which owns a top down order (ordCode) which is less than the 
	 * minimum of the ordCodes of the implicit processes facets
	 * (this is because we want to reflect the logic of processes applicability)
	 * @param baseTerm
	 * @param facetIndex
	 * @param facetCode
	 */
	private void forbiddenProcessesOrderForDerivativesCheck ( Term baseTerm, 
		...
		// if the process facet is found then check if its ord code is less than the minimum implicit ord code
	...
	/**
	 * FOURTH WARNING CHECK
	 * Check if more than one process with the same ord code is chosen
	 * @param addedProcesses the list of processes that were added to the base term by the user (with describe)
	 */
		...
		for( double ordCode : uniqueOrdCodes ) {
			...
			if ( Collections.frequency( ordCodes, ordCode) > 1 ) {
				...
				for ( int i = 0; i < ordCodes.size(); i++ ) {
					...
					// unless the last process code is appended, add a - to separate the processes codes
	...
	/**
	 * FIFTH WARNING CHECK
	 * Check if processes with ord code x.1, x.2, x.3 ... are added to the baseterm. If multiple processes of this type with the same 
	 * integer part are added, then rise a warning! These processes create another derivative which is already present in the list.
	 * Also the implicit processes are taken into account. NOTE: if only implicit facets have these properties, then no warning are risen. 
	...
	private void decimalOrderCheck ( Term baseTerm, ArrayList< ForbiddenProcess > implicitProcesses,  
		...
		for ( int i = 1; i <= maxValue; i++ ) {
			...
			// array list to save the forbidden processes which have ordcodes with the integer part = i
			ArrayList< ForbiddenProcess > imp = new ArrayList<>();
			ArrayList< ForbiddenProcess > exp = new ArrayList<>();

			// array list to save the processes fractional part of the ord codes which have the integer part = i
			...
			// get all the ord codes related to the processes involved (implicit and explicit) and
			...
			// if at least one facet is implicit and one is explicit and they have a different ord code => warning
			// we check if they have a different ord code because in that case the mutually exclusive warning is risen
			...
			if ( ( firstCheck || secondCheck ) && uniqueAllCodes.size() > 1 ) {
				...
				// for each element in the exp processes with ord code with decimal and integer part = i
				...
				// for each element in the imp processes with ord code with decimal and integer part = i
				...
				// print warning: these processes generate a derivative which is already existing
	...
	/**
	 * SIXTH WARNING CHECK
	 * Check if the selected baseTerm belongs to the reporting hierarchy or to the exposure hierarchy. If not => warning
	 * you are using a non reportable term.
	 * @param baseTerm
	 */
	...
	private void nonSpecificTermCheck ( Term term, boolean stdOut ) {
		...
		// if the term is a non-specific one, rise a warning (semaphore green, text yellow)
	...
	/**
	 * EIGHT CHECK
	 * Check if the "processed" facet is selected or not
	 * @param facet
	 */
	...
	/**
	 * TENTH CHECK
	 * Check if the user added a source to a derivative or to a raw commodity
	 * @param baseTerm
	 * @param facetIndex
	 * @param facetCode
	 */
	 ...
	 * is already specified, then a source cannot be used. Warnings are raised in the warning situations.
	 * @param baseTerm
	 * @param fullFacetCode
	 */
	...
	private ArrayList< Double > getDecimalOrdCodes ( ArrayList< ForbiddenProcess > forbiddenProcesses ) {
		...
		for ( int i = 0; i < forbiddenProcesses.size(); i++ ) {
			...
			// if the integer version of the ord code lost a bit of information => it is a decimal ordcode
	 ...
	 * @param event
	 * @param postMessageString, string to be printed between brackets after the warning
	 ...
	 * Update the current warning level to the highest retrieved until now
	 * Update the semaphore and text accordingly to the warning level
	 * @param event: the event which causes the printWarning
	 * @param postMessageString: a string which is attached to the end of the message (used for additional info)
	...
	private void printWarning( WarningEvent event, String postMessageString, 
		...
		// if the warning level of this message is greater than or equal the previous ones
	...
	/**
	 * Open the file filename and retrieve the warning messages (ID and message)
	 * the file must be a CSV file with 3 fields:  idMessage, description of the warning event, message
	 * @param filename
	...
	private ArrayList<WarningMessage> loadWarningMessages( String filename ) {
		try {
			...
			while ( ( line = bufferedReader.readLine() ) != null ) {
				...
				else if ( warningLevelToken.toLowerCase().replace(" ", "").equals("low") ) {  // if the level is set to LOW
				...
				else if ( textWarningLevelToken.toLowerCase().replace(" ", "").equals("low") ) {  // if the level is set to LOW
	...
	private class WarningMessage {
		...
		// the id of the message (used to connect the warning event to the warning message)
		public int getId() {
			return id;
		}
	...
	private static int createDefaultWarningMessagesFile ( String filename ) {
		...
		try {
			...
			// first message
			sb.append("1;if a hierarchy is selected as base term (describe function);Warning: the base term is a hierarchy!;LOW;LOW");
			sb.append("\r\n");

			// second message
			sb.append("2;if a non-hierarchy is selected as base term (describe function);The base term was successfully added!;NONE;NONE");
			sb.append("\r\n");

			// third message
			sb.append("3;if a forbidden process is chosen (the derivative should be used);You are trying to generate an already existing derivative!;HIGH;HIGH");
			sb.append("\r\n");

			// fourth message
			sb.append("4;if a derivative is described with a process facet with an ordCode value less than the implicit ordCode;"
			...
			//sixth message
			sb.append("6;if more than one process with the same ordCode is chosen (mutually exclusive property violated);"
			...
			//eight message
			sb.append("8;if a non exposure hierarchy is selected as base term (describe);You have selected a non-exposure hierarchy!;LOW;LOW");
			sb.append("\r\n");
	...
	public static ArrayList<ForbiddenProcess> loadForbiddenProcesses( String filename ) {
		...
		try {
			...
			while ( ( line = bufferedReader.readLine() ) != null ) {
				...
				// get the ord code (an ord code could be also with decimals to manage some particular situations)
				try {
	 ...
	 * raised if the user applies processes to raw commodities in order to create a derivative
	 * and that derivative is already present in the list (with its own code).
	 * Moreover, the processes have to be applied with a particular order, therefore we check
	 * that order with the ordCode field.
	 * @author Valentino
	 *
	 */
