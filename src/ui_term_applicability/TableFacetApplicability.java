...
>>>>>>> 574ffe363e78d250cf6350ff4ea89f2f48352380
		...
		GlobalUtil.addStandardColumn ( tableViewer, new AttributeColumnLabelProvider(), Messages.getString("TableFacetApplicability.AttributeColumn") ); //$NON-NLS-1$
		...
		GlobalUtil.addStandardColumn ( tableViewer, new InheritedColumnLabelProvider(), Messages.getString("TableFacetApplicability.InheritedColumn") ); //$NON-NLS-1$
	}
	...
	private class AttributeColumnLabelProvider extends ColumnLabelProvider {
		...
		public String getText ( Object element ) {
			...
			//return "[" + att.getAttribute().getCode() + "]" + att.getAttribute().getExtendedName(); //$NON-NLS-1$ //$NON-NLS-2$
			return "";
		}
	}
	...
	private MenuItem addAddMenuItem ( Menu menu, final Term term ) {
		...
		addFacet.setText( Messages.getString("TableFacetApplicability.AddCmd") ); //$NON-NLS-1$

		// only if a term is selected I will create the subMenu 
		/*if ( term != null ) {

			// the following is the sub-menu for the add operation 
			final Menu addFacetsMenu = new Menu( parent.getShell() , SWT.DROP_DOWN );

			// crash here
			//final MenuItem[] addFacetsMenuItems = new MenuItem[foodexDAO.Attributes.size()];
			int c = 0;
			for ( Iterator< Attribute > i = foodexDAO.Attributes.iterator(); i.hasNext() ; ) {

				// get the current attribute
				Attribute attr = i.next();

				// add the menu item related to the current available attribute
				// if the attribute is not already present in the term attribute
				if ( !term.hasAttribute( attr ) )
					addAttributeMenuItem ( addFacetsMenu, term, attr );

			}
			// If there are no attribute to use the function add is disabled 
			if ( c > 0 ) {
				addFacet.setMenu( addFacetsMenu );
				addFacet.setEnabled( true );
			} else
				addFacet.setEnabled( false );
		}

		// if the term is null disable the add item
		if ( term == null )
			addFacet.setEnabled( false );
		*/
		return addFacet;
	...
	private MenuItem addRemoveMenuItem ( Menu menu, final Term term ) {
		...
		menuItem.setText( Messages.getString("TableFacetApplicability.RemoveCmd") ); //$NON-NLS-1$
