...
/**
 * Table which is used to display the selected 
 * descriptors in the FormSelectTerm (multiple selection case only)
 * @author avonva
 *
 */
public class TableSelectedDescriptors {
	...
	private Menu createMenu ( Shell shell, final TermTable table ) {
		...
		// set menu item icon
		Image removeIcon = new Image( Display.getCurrent() , this.getClass().getClassLoader().getResourceAsStream(
				"remove-icon.png" ) );
