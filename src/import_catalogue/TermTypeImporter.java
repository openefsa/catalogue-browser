...
/**
 * Class which is used to import the term types of a catalogue.
 * The term types are stored inside the scopenote of the 
 * attribute named "termType" with the following syntax:
 * 
 * {@code This is the type of the term$r=raw commodity$f=facets$...}
 * 
 * the first element is the scopenote itself, while the other
 * elements in a $ separated format are the term types codes and
 * labels. In particular the element before the equal sign is the
 * term type code (which is the one used in the catalogue workbook).
 * The second element instead is the text which is displayed
 * for the corresponding term type code.
 * @author avonva
 *
 */
public class TermTypeImporter {
	...
	/**
	 * Import the term types. Note that you should import the
	...
	/**
	 * Create the table related to the term types retrieved 
	 * from the term type scopenote
	 */
