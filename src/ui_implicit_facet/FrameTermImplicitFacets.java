...
/**
 * The third tab of the main page of the foodex browser. It includes the implicit facets tree.
 * This tab is also used into the describe window to add new facets
 * @author avonva
 *
 */
public class FrameTermImplicitFacets implements Observer {
	...
	private Listener addDescriptorListener;     // called each time that a facet is added (if multiple, it is called multiple times)
	private Listener removeDescriptorListener;  // called each time that a facet is removed
	private Listener updateListener;            // called at the end of an adding operation, useful if we have to do something only once in multiple selection cases
	...
	/**
	 * Create the implicit facet tree. newFacetType is used to define the type of the facets descriptors which
	 * are added to the already present facets descriptors. Are they implicit facets (editing mode, we modify the
	 * implicit facets of a term) or explicit facets (describe, we are adding facet to make a code)
	 * @param parent
	 * @param newFacetType
	 */
	...
	private Menu addImplicitFacetsMenu ( final Shell parent, final TreeViewer implicitFacets ) {
		...
		addImplicitFacet.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected ( SelectionEvent e ) {
				...
				Object selectedElem = ( (IStructuredSelection) ( implicitFacets
						.getSelection() ) ).getFirstElement();

				Attribute facetCategory;
				...
				for ( int i = 0; i < sf.getSelectedTerms().size(); i++ ) {
					...
					TermAttribute ta = new TermAttribute ( term, 
							facetAttr, FacetDescriptor.getFullFacetCode( descriptor, facetCategory) );
					
					// create the new facet descriptor using the facet category and the term full code
		...
		removeImplicitFacet.addSelectionListener( new SelectionAdapter() {
			...
			public void widgetSelected ( SelectionEvent e ) {
				...
				// ( we can have only selected a descriptor tree item, since remove is disabled for
				// facet categories )
				DescriptorTreeItem treeItem = (DescriptorTreeItem) ( (IStructuredSelection) ( implicitFacets
						.getSelection() ) ).getFirstElement();
				
				// remove the descriptor contained in the selected tree node from the implicit facets
	...
	public boolean[] isAddableRemovable( Term baseTerm, Object facetCategory ){
		...
		if ( facetCategory instanceof Attribute ) {
			...
			// if we selected a facet descriptor we get the cardinality from the attribute contained into the
			// descriptor, which is the facet category
			cardinality = ( ( DescriptorTreeItem ) facetCategory).getDescriptor()
					.getFacetCategory().getSingleOrRepeatable();
